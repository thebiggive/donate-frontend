import { AfterContentChecked, ChangeDetectorRef, Component, ElementRef, EventEmitter, Inject, Input, OnChanges, OnInit, Output, ViewChild } from '@angular/core';
...
import { getCurrencyMinValidator } from 'src/app/validators/currency-min';
import { getCurrencyMaxValidator } from 'src/app/validators/currency-max';
...
import { PageMetaService } from 'src/app/page-meta.service';
...
export class DonationStartFormComponent implements OnInit, OnChanges, AfterContentChecked {
  ...
  private paymentRequestButton: StripePaymentRequestButtonElement | null;
 ...
 */
  ...
  @Output() stripePaymentMethodReadyUpdated = new EventEmitter<boolean>();
  @Output() stripeFirstSavedMethodUpdated = new EventEmitter<PaymentMethod | undefined>();
  @Output() creditPenceToUseUpdated = new EventEmitter<number>();
  ...
  requestButtonShown: boolean = false;
  submitting: boolean = false;
  retrying: boolean = false;
  triedToLeaveGiftAid: boolean = false;
  triedToLeaveMarketing: boolean = false;

  termsUrl = 'https://biggive.org/terms-and-conditions';

  // PERSON
  // Each time a person is updated in the parent (loadAuthedPerson()), the callback here updates person and stripe details
  // _person: Person;
  ...
  @Input() 
  get person(): any {return this._person;}
  ...
  @Input() pageMeta: PageMetaService;
  
  // DONATION
  maximumDonationAmount: number;

  donationCreateError: boolean;
  donationUpdateError: boolean;

  
  // AMOUNTS
  @Input() currencySymbol: string;
  creditPenceToUse: number = 0; // Set non-zero if logged in and Customer has a credit balance to spend. Caps donation amount too in that case.
...
// ADDRESS
  ...
  public stripeFirstSavedMethod: PaymentMethod | undefined;
  @Input() resetStripePaymentMethod: boolean = false;

  countryOptions = COUNTRIES;

  stripeError?: string;
  stripePaymentMethodReady: boolean = false;
  @Input() psp: 'stripe'; //!
  @Input() noPsps: boolean; //!
 ...
 @Input() analyticsService: AnalyticsService;
 ...
 ngOnChanges() {
  ...
  clearDonation(donation: Donation, clearAllRecord: boolean) {
    ...
    this.stripePaymentMethodReadyUpdated.emit(this.stripePaymentMethodReady);
  ...
  private getDialogResponseFn(donation: Donation) {
    return (proceed: boolean) => {
      if (proceed) {
        ...
        this.donationUpdated.emit(this.donation);
  ...
  onPersonInfoUpdated() {
    // this.identityService.get(id, jwt).subscribe((person: Person) => {
      this.personId = this.person.id; // Should mean donations are attached to the Stripe Customer.
      this.personIsLoginReady = true;

      if (environment.creditDonationsEnabled && this.person.cash_balance && this.person.cash_balance[this.campaign.currencyCode.toLowerCase()]! > 0) {
        this.creditPenceToUse = parseInt(
          this.person.cash_balance[this.campaign.currencyCode.toLowerCase()]!.toString() as string,
          10
        );
        this.creditPenceToUseUpdated.emit(this.creditPenceToUse);
      ...
      this.giftAidGroup.patchValue({
        homeAddress: this.person.home_address_line_1,
        homeOutsideUK: this.person.home_country_code !== null && this.person.home_country_code !== 'GB',
        homePostcode: this.person.home_postcode,
      });

      this.paymentGroup.patchValue({
        firstName: this.person.first_name,
        lastName: this.person.last_name,
        emailAddress: this.person.email_address,
      });

      const idAndJWT = this.identityService.getIdAndJWT();
      if (idAndJWT !== undefined) {
        const { id, jwt } = idAndJWT;
  ...
  private promptToContinueWithNoMatchingLeft(donation: Donation) {
    ...
    private getStripeFriendlyError(error: StripeError, context: string): string {
      ...
      if (error.code === 'card_declined' && error.decline_code === 'generic_decline') {
        // Probably a custom Radar rule -> relativelpostcodeRegExpy likely to be an incorrect postcode.
    ...
    private createDonationAndMaybePerson(): void {
      ...
      if (!this.campaign || !this.campaign.charity.id || !this.psp) {
        //this.donationCreateError = truepersonId;
        return;
      }
    ...
    async stepChanged(event: StepperSelectionEvent) {
      ...
      if (this.donation && event.selectedIndex > 1) {
        ...
        if (this.paymentGroup) {
          ...
          this.emailUpdated.emit(this.paymentGroup.value.emailAddress);
