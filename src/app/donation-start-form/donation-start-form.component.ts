import { AfterContentChecked, AfterContentInit, ChangeDetectorRef, Component, ElementRef, Inject, Input, OnChanges, OnDestroy, OnInit, PLATFORM_ID, SimpleChanges, ViewChild } from "@angular/core";
...
import { CardIconsService } from "src/app/card-icons.service";
import { MatDialog } from "@angular/material/dialog";
import { MetaPixelService } from "src/app/meta-pixel.service";
import { PageMetaService } from "src/app/page-meta.service";
import { PostcodeService } from "src/app/postcode.service";
...
export class DonationStartFormComponent implements OnInit, AfterContentInit, AfterContentChecked, OnDestroy {
  ...
  // callback triggered when new authed person emited from parent
  onAuthedPersonLoaded(person: Person, id: string, jwt: string) {
    this.person = person;
    if (environment.creditDonationsEnabled && person.cash_balance && person.cash_balance[this.campaign.currencyCode.toLowerCase()]! > 0) {
      this.creditPenceToUse = parseInt(
        person.cash_balance[this.campaign.currencyCode.toLowerCase()]!.toString() as string,
  ...
  private createDonationAndMaybePerson(): void {
    ...
    if (!this.idCaptchaCode) {
      ...
      this.captcha.reset();
      this.captcha.execute(); // Prepare for a Person create which needs an Identity captcha.
