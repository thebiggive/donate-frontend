...
export class RegisterComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private async doRegistrationAndLogin(captchaResponse: string | undefined = undefined) {
    const emailAddress =
      (this.verificationCodeSupplied && this.verificationLinkSentToEmail) || this.registrationForm.value.emailAddress;
  ...
  private login(captchaResponse: string | undefined) {
    ...
    this.identityService
      ...
      .subscribe({
        ...
        error: (error) => {
          this.error =
            'Auto login: ' + (error.error.description !== undefined ? error.error.description : error.message) ||
            'Unknown error';
          this.processing = false;
        },
      });
  ...
  async verificationCodeEntered(verificationCode: string) {
    ...
    const tokenValid = await this.identityService.checkNewAccountEmailVerificationTokenValid({
      ...
      emailAddress: this.verificationLinkSentToEmail,
    });
    ...
    if (!tokenValid) {
      this.error =
        "Sorry, we couldn't confirm that code. Please try again, or refresh the page and re-enter your email address to generate a new code.";
      this.processing = false;
      return;
    }
