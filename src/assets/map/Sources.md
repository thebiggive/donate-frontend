...
## Next steps for using the map layers

@todo:

1. Figure out the overlap. If e.g. Hackney is in `counties` but also `localAuthorities` then do we definitely need the former?
2. Add support for project highlights of areas from every file except `localAuthorities.geojson` which is used in the CampaignDetailsComponent demo.
