 ...
 */
function buildCspDirectives(externalScriptNonce: string) {
  return {
...
app.use('**', async (req, res, next) => {
  ...
  // If legacy browser detected, rewrite script sources in the rendered HTML to ES5 version.
  if (useLegacy) {
    let html = await response.text();
    html = html.replace(/\/browser\//g, '/browser-es5/');
  ...
  // Standard response stream for modern browsers
