...
try {
  ...
  if (fs.existsSync(BROWSER_ES5_DIR)) {
    ...
    // Check for JS files
    const jsFiles = fs.readdirSync(BROWSER_ES5_DIR).filter((file) => file.endsWith(".js"));
    ...
    const htmlFiles = fs.readdirSync(BROWSER_ES5_DIR).filter((file) => file.endsWith(".html"));
